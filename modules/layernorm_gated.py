...
#####
# so the functions here are over-complicated for efficiency's sake, i can just use my regular Norm
# the silu gate part is just to combine the gate near the end of the mamba block with the norm for efficiency reasons
# then the group size is just splitting up the vectors to be a max of 64kb again for efficiency reasons
####



# so the two parts of this function that I *might* care about are the silu gate and the group split. i have a feeling the latter is for vision models
...
# basically just a bunch of shape setup to ensure efficiency given a max size to actually RMSNorm
...
# helps keep track of gradients correctly since that gets messed up by the cusom kernels
